...
for dim_constant in range(0, 101):
  ...
  for seed in range(NUM_EXPERIMENTS):
    ...
    sigma = approx_sigma_bound_nb(A_ndarray, x_ndarray, N_FEATURES)

    x_ndarray = x_ndarray / (50 * sigma)
    ...
    cur_epsilon = np.abs(z_hat[bandit_topk_indices] - numpy_z[4]) / numpy_z[4]
...
plt.plot(dimension_list, np.log(budget_list), "r--.", label="adaptive_softmax")
plt.plot(dimension_list, np.log(N_CLASSES * np.array(dimension_list)), "b--.", label="naive")
plt.legend()
plt.xlabel("dimension(n_features)")
plt.ylabel("number of samples taken")
plt.savefig("sample_complexity_plot(true_fixed_sigma).png")