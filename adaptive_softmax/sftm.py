...
class SFTM:
  ...
  def best_arms(self, dlt: float, bta: float, sig2: float, k: int=1) -> int:
    n = self.n
    d = self.max_pulls
    T0 = min(d, 17 * (bta ** 2) * sig2 * log(6 * n / dlt))
    ...
    while True:
      ...
      confidence_interval = 0 if num_pulls >= self.bandits.max_pulls \
        else sqrt(2 * sig2 * log(6 * n * log(d) / dlt) / num_pulls)
  ...
  def estimate_arm_logits(self, arms: np.ndarray, bta: float, eps: float, dlt: float, sig2: float) -> float:
    T = min(self.max_pulls, 32 * (sig2) * (bta ** 2) * log(2 / dlt) / (eps ** 2))
  ...
  def log_norm_estimation(self, bta: float, eps: float, dlt: float, sig2: float) -> float:
    ...
    n = self.n
    T0 = min(self.max_pulls, 17 * (bta ** 2) * sig2 * log(6 * n / dlt))
    C = np.sqrt(2 * sig2 * log(6 * n / dlt) / T0)
    
    mu_hat = self.bandits.pull(np.arange(n), it=T0)
    ...
    it = np.minimum(it, self.max_pulls)
