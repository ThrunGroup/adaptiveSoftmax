...
def compute_mip_batch_topk_ver2_warm(atoms, query, sigma, delta, batch_size=16, k=1, mu=None, budget_vec=None):
    ...
    max_mu = mu[max_index]
    C = np.divide(sigma * np.sqrt(2 * np.log(4 * n_atoms * d_used**2 / delta)), d_used + 1) if d_used is not None else np.zeros(n_atoms)

    solution_mask = np.ones(n_atoms, dtype="bool") & (mu + C >= max_mu - C[max_index]) if mu is not None else np.ones(n_atoms, dtype="bool")
    ...
    while (len(solutions) > brute_force_threshold and found_indices_num < k and np.max(d_used) < dim - batch_size):  #TODO: computing max everytime may degrade performance

        tmp = np.empty(np.sum(solution_mask))

        for i, atom_index in enumerate(solutions):
          tmp[i] = atoms[atom_index, d_used[atom_index] : d_used[atom_index] + batch_size] @ query[d_used[atom_index] : d_used[atom_index] + batch_size]

        mu[solutions] = np.divide(np.multiply(d_used[solution_mask], mu[solutions]) + tmp * dim, d_used[solution_mask] + batch_size)
        n_samples += len(solutions) * batch_size

        C = np.divide(sigma * np.sqrt(2 * np.log(4 * n_atoms * d_used**2 / delta)), d_used + 1) #TODO: update confidence bound. This is when we're sampling one A_iJ * x_J at each round. Can and should be tighter than this -> divide with + batch size somehow?
        ...
        if len(solutions) <= k - found_indices_num:
          ...
          best_ind[found_indices_num : found_indices_num + len(solutions)] = solutions
          found_indices_num += len(solutions)
          solution_mask = np.logical_xor(solution_mask_before, solution_mask) #TODO: Does xor work even in the worst case scenario? or should we revive all candidates?
          solutions = np.nonzero(solution_mask)[0]
          max_index = solutions[np.argmax(mu[solution_mask])]
          max_mu = mu[max_index]
...
def ada_softmax(A, x, beta, epsilon, delta, sigma, k, bruteforce=False, return_estimates=False):
    ...
    S_hat, mu_hat, budget_vec = estimate_softmax_normalization_warm(A, x, beta, epsilon / 2, delta / 3, sigma, bruteforce=bruteforce)

    normalization_budget = np.sum(budget_vec)

    #print("normalization budget:", normalization_budget)

    best_index_hat, budget_mip, mu_hat, budget_vec = compute_mip_batch_topk_ver2_warm(A, x, sigma, delta / 3, batch_size=16, k=k, mu=mu_hat, budget_vec=budget_vec)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, top_k=None, temperature=1, targets=None, is_adaptive=False):
        device = idx.device
        b, t = idx.size()
        print("=> t is ", t)
        ...
        if targets is not None:
            ...
            if is_adaptive:
                ...
                for batch in range(x.shape[0]):
                    best_indices, prob, budget = ada_softmax(A_matrix, x[batch, -1], beta, epsilon, delta, sigma, k,)
    ...
    @torch.no_grad()
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, is_adaptive=False):
        """
