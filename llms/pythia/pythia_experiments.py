...
def get_adaptive_forward(model) -> Callable:
    ...
    def adaptive_forward(
        ...
        # devs = np.abs(elems-mu[:,None])/stds[:,None]
        # plt.scatter(x,devs.sum(axis=0)/n)
        ...
        plt.savefig("unifMixFactor to variance pythia.png")
...
def run_experiment(
    ...
    max_length = naive_model.config.max_length
